	...
	maybeDisplayLinkedInNotice = () =>
		this.connectionNeedsReauth() && (
			<Notice className="jetpack-publicize-notice" isDismissible={ false } status="error">
				<p>
					{ __(
						'The LinkedIn connection requires reauthentication to continue working properly. Visit your Sharing page to do it.'
					) }
				</p>
				<ExternalLink href={ getSharingLink() }>{ __( 'Go to Sharing page' ) }</ExternalLink>
			</Notice>
		);
